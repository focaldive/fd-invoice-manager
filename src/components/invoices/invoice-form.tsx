...
import { createRecurringTemplate } from "@/server/actions/recurring";
...
export function InvoiceForm({ clients: initialClients, settings, invoice }: Props) {
  const router = useRouter();
  const { registerGuard, unregisterGuard } = useNavigationGuard();
  const isEdit = !!invoice;

  const [clients, setClients] = useState<Clients>(initialClients);
  const [invoiceNumber, setInvoiceNumber] = useState(invoice?.invoiceNumber ?? "");
  const [clientId, setClientId] = useState(invoice?.clientId ?? "");
  const [dateOfIssue, setDateOfIssue] = useState(
    invoice?.dateOfIssue ?? toLocalDateString(new Date()),
  );
  const [dateDue, setDateDue] = useState(
    invoice?.dateDue ??
      toLocalDateString(new Date(Date.now() + (settings?.defaultPaymentTerms ?? 4) * 86400000)),
  );
  const [category, setCategory] = useState(invoice?.category ?? "other");
  const [currency, setCurrency] = useState(
    invoice?.currency ?? settings?.defaultCurrency ?? "LKR",
  );
  const [taxPercentage, setTaxPercentage] = useState(
    invoice ? Number(invoice.taxPercentage) : Number(settings?.defaultTaxPercentage ?? 0),
  );
  const [discountType, setDiscountType] = useState<"percentage" | "fixed">(
    invoice && Number(invoice.discountAmount) > 0 && Number(invoice.discountPercentage) === 0
      ? "fixed"
      : "percentage",
  );
  const [discountPercentage, setDiscountPercentage] = useState(
    invoice ? Number(invoice.discountPercentage) : 0,
  );
  const [discountFixedAmount, setDiscountFixedAmount] = useState(
    invoice && Number(invoice.discountPercentage) === 0 ? Number(invoice.discountAmount) : 0,
  );
  const [notes, setNotes] = useState(invoice?.notes ?? settings?.defaultNotes ?? "");

  const initialItems: LineItem[] = invoice?.items.length
  ...
  const isDirty = useCallback(() => {
    ...
    if (items.some((i) => i.description.trim() || i.unitPrice > 0)) return true;
    return false;
  }, [clientId, items]);
  ...
  useEffect(() => {
    ...
    if (!client) return;
    let cancelled = false;
    generateNextInvoiceNumber(client.name, dateOfIssue).then((num) => {
      if (!cancelled) setInvoiceNumber(num);
    });
    return () => {
      cancelled = true;
    };
  }, [clientId, dateOfIssue, clients, isEdit]);
  ...
  function addItem() {
    setItems([
      ...items,
      { description: "", quantity: 1, unitPrice: 0, amount: 0, sortOrder: items.length },
    ]);
  }
  ...
  function validateForm(): boolean {
    ...
    if (!invoiceNumber) {
      toast.error("Invoice number not generated yet. Please wait a moment.");
      return false;
    }
  ...
  async function persistInvoice(status: "draft" | "sent"): Promise<string | null> {
    const payload = {
      invoiceNumber,
      clientId,
      dateOfIssue,
    ...
    try {
      if (isEdit && invoice) {
        await updateInvoice(invoice.id, payload, itemsPayload);
        return invoice.id;
      } else {
        const created = await createInvoice(payload, itemsPayload);
        return created.id;
      }
    } catch (err) {
  ...
  async function persistRecurring(linkToInvoiceId: string) {
    try {
      await createRecurringTemplate({
        template: {
          ...
          taxPercentage,
          discountPercentage: discountType === "percentage" ? discountPercentage : 0,
          discountAmount: discountType === "fixed" ? discountFixedAmount : 0,
          notes: notes || null,
          category,
      ...
      toast.error(err instanceof Error ? err.message : "Invoice saved but failed to create recurring template");
    }
  }
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <Input
                  value={invoiceNumber}
                  readOnly
                  placeholder={isEdit ? "" : "Select a client first"}
                  className="mt-1.5 bg-secondary font-mono text-sm"
                />
              </div>
              <div>
              ...
              {items.map((item, idx) => (
                <div key={idx} className="grid gap-2 sm:grid-cols-12 items-start rounded-lg border border-border p-3 sm:border-0 sm:p-0">
                  ...
                  <div className="sm:col-span-2 flex items-center justify-between sm:justify-end h-9 px-3 text-sm font-mono font-semibold">
                    ...
                    {formatAmount(Number(item.quantity) * Number(item.unitPrice))}
                  </div>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="flex flex-col gap-2 pt-4">
            <Button
              ...
              disabled={!clientId}
              className="w-full rounded-full border border-primary text-primary hover:bg-primary/20"
            >
              <Save className="mr-1.5 h-3.5 w-3.5" /> Save as Draft
            </Button>
            {!clientId && (
              <p className="text-[11px] text-muted-foreground text-center -mt-1">
                Select a client first to save as draft
              </p>
            )}
            <Button
              variant="outline"
